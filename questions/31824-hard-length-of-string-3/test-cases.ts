...
type Reped<S extends string, C extends Signum, R extends string = ''> =
...
type t5 = Reped<t4, 10>;
type t6 = Reped<t5, 10>;
type Signums<N extends string, Acc extends readonly Signum[] = []> =
	N extends `${infer Head extends Signum}${infer Rest}` ? Signums<Rest, [...Acc, Head]> : Acc;
type Gened<N extends string> =
	Signums<N> extends (
		[
		...
		`${''}${Reped<t6, N6>}${Reped<t5, N5>}${Reped<t4, N4>}${Reped<t3, N3>}${Reped<
			t2,
			N2
		>}${Reped<t1, N1>}${Reped<t0, N0>}`
	:	never;
